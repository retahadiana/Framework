...
<div class="container" style="max-width:400px;margin:60px auto;background:#fff;border-radius:14px;box-shadow:0 2px 8px rgba(0,0,0,0.08);padding:32px 28px 24px 28px;">
    ...
    <p>Apakah Anda yakin ingin menghapus ras <strong>{{ $item->nama_ras }}</strong>?</p>
    <form action="{{ route('ras-hewan.destroy', $item->idras_hewan) }}" method="POST">
        @csrf
        @method('DELETE')
        <a href="{{ route('ras-hewan.index') }}" class="btn btn-secondary" style="background:#6c757d;color:#fff;border:none;border-radius:6px;padding:12px 0;font-size:1.1rem;font-weight:600;box-shadow:0 1px 4px rgba(108,117,125,0.08);transition:background 0.2s;">Batal</a>
