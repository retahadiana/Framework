...
<div style="background: #f6fbfd; min-height: 100vh; padding-top: 40px;">
    <div class="ras-create-card">
        <div class="ras-create-title">Tambah Ras Hewan</div>
        <form action="{{ route('ras-hewan.store') }}" method="POST">
            ...
            <input type="text" name="nama_ras" class="ras-create-input @error('nama_ras') is-invalid @enderror" value="{{ old('nama_ras') }}">
            @error('nama_ras')
                <div class="invalid-feedback" style="color:#e74c3c;text-align:left;margin-bottom:10px;">{{ $message }}</div>
            @enderror
            <div class="ras-create-btn-row">
                <button type="submit" class="ras-create-btn-simpan">Simpan</button>
