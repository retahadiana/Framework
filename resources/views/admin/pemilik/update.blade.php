...
<div style="background: #f6fbfd; min-height: 100vh; padding-top: 40px;">
	<div class="pemilik-create-card">
		...
		<form action="{{ route('pemilik.update', $item->idpemilik) }}" method="POST" style="text-align:left;">
			...
			<textarea name="alamat" class="pemilik-create-input @error('alamat') is-invalid @enderror" style="height:80px;resize:vertical;">{{ old('alamat', $item->alamat) }}</textarea>
			@error('alamat')
				<div class="invalid-feedback" style="color:#e74c3c;text-align:left;margin-bottom:10px;">{{ $message }}</div>
			@enderror
			<div class="pemilik-create-btn-row" style="margin-top:30px;">
				<button type="submit" class="pemilik-create-btn-simpan" style="font-size:1.15rem;">Update</button>
				<a href="{{ route('pemilik.index') }}" class="pemilik-create-btn-batal" style="font-size:1.15rem;">Batal</a>
			</div>
		</form>
	</div>
</div>
