...
<div class="page-section" style="background: #f6f9fb; min-height: 100vh;">
    <div class="card" style="background: #fff; border-radius: 24px; box-shadow: 0 2px 16px rgba(0,0,0,0.07); padding: 40px 32px; max-width: 600px; margin: 60px auto;">
        <h2 style="text-align: center; color: #22346a; font-weight: 700; margin-bottom: 32px; font-size: 2rem;">Tambah Kode Tindakan & Terapi</h2>
        <form action="{{ route('kode-tindakan-terapi.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="kode" class="form-control @error('kode') is-invalid @enderror" value="{{ old('kode') }}" style="border-radius: 12px; border: 1.5px solid #319da7; font-size: 1.15rem; padding: 12px 16px; margin-top: 8px;">
                @error('kode')
            ...
            <div class="mb-4">
                ...
                <input type="text" name="deskripsi_tindakan_terapi" class="form-control @error('deskripsi_tindakan_terapi') is-invalid @enderror" value="{{ old('deskripsi_tindakan_terapi') }}" style="border-radius: 12px; border: 1.5px solid #319da7; font-size: 1.15rem; padding: 12px 16px; margin-top: 8px;">
            ...
            <div class="mb-4">
                ...
                <select name="idkategori_klinis" class="form-control @error('idkategori_klinis') is-invalid @enderror" style="border-radius: 12px; border: 1.5px solid #319da7; font-size: 1.15rem; margin-top: 8px;">
                    ...
                    @foreach($kategoriKlinisList as $kategoriKlinis)
                        <option value="{{ $kategoriKlinis->idkategori_klinis }}" {{ old('idkategori_klinis') == $kategoriKlinis->idkategori_klinis ? 'selected' : '' }}>{{ $kategoriKlinis->nama_kategori_klinis }}</option>
                    @endforeach
                </select>
