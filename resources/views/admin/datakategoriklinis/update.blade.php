...
<div class="page-section" style="background: #f6f9fb; min-height: 100vh;">
	<div class="card" style="background: #fff; border-radius: 24px; box-shadow: 0 2px 16px rgba(0,0,0,0.07); padding: 40px 32px; max-width: 480px; margin: 60px auto;">
		<h2 style="text-align: center; color: #22346a; font-weight: 700; margin-bottom: 32px; font-size: 2rem;">Edit Kategori Klinis</h2>
		<form action="{{ route('kategori-klinis.update', $kategoriKlinis->idkategori_klinis) }}" method="POST">
			...
			<div class="mb-4">
				...
				<input type="text" name="nama_kategori_klinis" class="form-control @error('nama_kategori_klinis') is-invalid @enderror" value="{{ old('nama_kategori_klinis', $kategoriKlinis->nama_kategori_klinis) }}" style="border-radius: 12px; border: 1.5px solid #319da7; font-size: 1.15rem; padding: 12px 16px; margin-top: 8px;">
